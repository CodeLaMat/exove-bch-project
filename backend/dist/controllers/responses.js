...
const addResponse = async (req, res) => {
  ...
  for (const response of responsePack.totalResponses) {
    const employee = await user_1.default.findById(
      response.employeeTakingSurvey
    );
    if (
      (employee === null || employee === void 0
        ? void 0
  ...
  for (const { question, response } of allResponses) {
    ...
    const currentEmployeeResponse = employeeTakingSurvey.allResponses.find(
      (response) => {
        var _a;
        return (
          ((_a = response.question._id) === null || _a === void 0
            ? void 0
            : _a.toString()) === question
        );
      }
    );
...
const getAllResponses = async (req, res) => {
  ...
  res
    ...
    .json({ responsePack, count: responsePack.length });
};
...
const updateResponse = async (req, res) => {
  ...
  if (!responsePack) {
    throw new errors_1.NotFoundError(
      `No surveyPack with id: ${responsePackId}`
    );
  }
  const updatedResponsePack = await responses_1.default.findOneAndUpdate(
    { _id: responsePackId },
    req.body,
